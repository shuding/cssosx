$(document).ready(function(){
	...
	$('#browser_btn').on('click', function(){
		...
		$('#browser_app').append('<div class="open_item_light"></div>');
		setTimeout(function(){
			$('#browser').fadeIn(0);
			$('#browser').mousedown();
		},300);
	...
	$('.dock li').on('click', function(){
		...
		icon.css({'-webkit-box-reflect':'none'});
		setTimeout(function(){
	...
	$('#browser_app').on('click', function(){
		$(this).append('<div class="open_item_light"></div>');
		setTimeout(function(){
			$('#browser').fadeIn(0);
			$('#browser').mousedown();
		},300);
	...
	var launchpad = 0;
	$('#launchpad_app').on('click', function(){
		if(!launchpad){
			$('.main_menubar').css('visibility','hidden');
			$('.launchpad_layout').fadeIn(200);
			setTimeout(function(){
				$('.launchpad').css('visibility','visible');
				$('.launchpad').css('-webkit-transform', 'scale(1.0)');
			...
			setTimeout(function(){
				$('.launchpad').css('visibility','hidden');
				$('.launchpad').css('-webkit-transform', 'scale(1.1)');
	...
	$('.launchpad_layout').on('click', function(){
		if(launchpad){
			setTimeout(function(){
				$('.main_menubar').css('visibility','visible');
			}, 200);
	...
	$('#volume').on('change', function(event){
		...
		if(tmp.value == 0){
			$('i.system_volume').removeClass('fa-volume-up');
			$('i.system_volume').removeClass('fa-volume-down');
			$('i.system_volume').addClass('fa-volume-off');
		}
		else if(tmp.value <= 40){
			$('i.system_volume').removeClass('fa-volume-off');
			$('i.system_volume').removeClass('fa-volume-up');
			$('i.system_volume').addClass('fa-volume-down');
		}
		else{
			$('i.system_volume').removeClass('fa-volume-off');
			$('i.system_volume').removeClass('fa-volume-down');
			$('i.system_volume').addClass('fa-volume-up');
		}
	});
	
	var dock_expand = Math.PI / 300,
		dock_min = 50,
		dock_max = 150,
		dock_min_max_dex = 50,			// (max-min)/2
		dock_min_max_avg = 100,			// (max+min)/2
		dock_range = 300;
	var dock_resize = 0;
	...
	function set_dock(){
		dock_resize = -1;
		f.css(dock, 'visibility','hidden');
		f.css(dock2, 'display','inherit');
		f.each(dockimg, function(kay, val){
			x = f.offset(dockimg[key]).left + dockimg[key].offsetWidth * .5;
			d = Math.abs(mouseX - x);
			w = d < dock_range ? dock_min_max_avg + (dock_min_max_dex * fastCos(d * dock_expand)) : dock_min;
			f.css(dockimg[key], 'width', w);
		});
		setTimeout(function(){
			f.each(dockimg, function(){
				x = f.offset(dockimg[key]).left + dockimg[key].offsetWidth * .5;
				d = Math.abs(mouseX - x);
				w = d < dock_range ? dock_min_max_avg + (dock_min_max_dex * fastCos(d * dock_expand)) : dock_min;
				f.css(dockimg[key], 'width', w);
		...
		setTimeout(function(){
			f.css(dock, 'visibility','visible');
			f.css(dock2, 'display','none');
			f.each(dockimg, function(){
				w0 = f.css(dockimg[key], 'width');
				f.css(dockimg[key], 'width', dock_min);
				f.css(dockimg[key], 'width', w0);
			});
		}, 30);
		setTimeout(function(){
			dock_resize = 2;
	...
	function renew_dock(){
		dockimg.each(function(){
			if(Math.abs($(this).width() - dock_min) > 5){
				$(this).animate({'width':dock_min}, 150);
			}
			else{
